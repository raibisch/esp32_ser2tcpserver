...
IPAddress local_IP(192, 168, 2, 102);
...
{
   uint8_t i_timeout = 0;
   while ( (WiFi.status() != WL_CONNECTED) and (WiFi.status() != WL_CONNECT_FAILED) and (i_timeout < 50) )
    {
      i_timeout++;
      Serial.println(".");
...
void StartRouterConnect()
{
  ...
  */
  ...
  char pass2[] = "###ud361309$$$";   // your network password
